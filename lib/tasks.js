...
gulp.task('makdoc:prerequisite', function(done){
    ...
    //check mandatory peer dependancies
    var peerPkg = require(findup('package.json'));

    if(!peerPkg) {
        throw new Error('Could not find the your project. Do you have a package.json file in your project?');
    }

    var deps = _.extend({}, peerPkg.dependencies, peerPkg.devDependencies);
    ...
    var need = [
            ...
            "handlebars",
            "dashbars"
        ].filter(function(e){ return !deps.hasOwnProperty(e);});

    if(need.length > 0){
        throw new Error(JSON.stringify(need) + ' are necessary in your project');
    }
...
gulp.task('makdoc:init', function(done) {
    ...
    require('marked').setOptions({
        highlight: makdoc.util.highlight(),
        gfm: true,
        tables: true,
        breaks: false,
        pedantic: false,
    ...
    function defer(){
        if(!vars.SOLIDS){
            var solids = _([vars.DOCS(),
                         ...
                         .unshift('!**/*.{md,hbs}.json') // model files
                         .unshift(vars.ALL())
                         .flatten()
                         .value();
...
gulp.task('makdoc:partials', function(){
    return gulp.src(makdoc.vars.PARTIALS())
        ...
        .pipe(makdoc.model.loadDirectoryModel())
        .pipe(makdoc.model.loadFileModel())
        ...
        .pipe(hbsFilter)
            .pipe(through.obj(function(file, enc, done){
                ...
                file.contents =
                    new Buffer(
                        template(
                            _.assign({}, makdoc._templateData,
                                        file.model)));

                done(null, file);
            }))
            .pipe(hbsFilter.restore())
        .pipe(makdoc.model.extractModelFromDocument())
        .pipe(makdoc.model.loadDirectoryModel())
        .pipe(makdoc.model.loadFileModel())
        .pipe(makdoc.model.finalize())
        .pipe(makdoc.model.contents())
        .pipe($.rename({extname:''})) //drop Extension before model.url
        .pipe(makdoc.model.url())
...
gulp.task('makdoc:styles', function() {
    ...
    return gulp.src(makdoc.vars.STYLES())
        ...
        //.pipe($.minifycss())
        .pipe(gulp.dest(path.join(makdoc.vars.DIST(), 'styles/')))
        .pipe($.connect.reload());
});
...
gulp.task('makdoc:scripts', function() {
    ...
    return gulp.src(makdoc.vars.SCRIPTS())
        ...
        .pipe(gulp.dest(path.join(makdoc.vars.DIST(), 'scripts/')))
        .pipe($.connect.reload());
});
...
gulp.task('makdoc:watch', function(done){
    ...
    gulp.watch(makdoc.vars.LAYOUTS(), ['makdoc:layouts']);
    gulp.watch(makdoc.vars.PARTIALS(), ['makdoc:partials']);
