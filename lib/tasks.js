 ...
 */
    ...
    gulp.task('makdoc:init', function(done) {
        ...
        function defer(){
            ...
            vars.DOCS = function(){
                return ignores.concat(arrayfy(vars.DOCS()));
            };
            vars.IMAGES = function(){
                return ignores.concat(arrayfy(vars.IMAGES()));
            };
            vars.SOLIDS = function(){
                return ignores.concat(arrayfy(vars.SOLIDS()));
            };
 ...
 */
    ...
    gulp.task('makdoc:resources', function(done){
        if( makdoc.profile === 'dev') {
            $.util.log('skip makdoc:resources when in dev profile');
            done();
        } else {
            return seq([
                    'makdoc:solid',
