...
function init(gulp, makdoc) {
    ...
    var _PLUGIN_NAME='gulp-makdoc-pipelines';
        ...
        return through.obj(function(file, enc, done){
            if(!file.isNull()){ // not directory
                for(var i=0; i < apps.length; i++){
                    var index = file.path.indexOf(apps[i]); // absolute path

                    if( index === 0 ){ // absolute path
                        var next = file.path.indexOf('/',
                                                     apps[i].length + 1);

                        if( next > 0 ) {
                            file.base = file.path.slice(0, next);
                        }
    ...
    function templates(glob){
        return gulp.src(glob)
            ...
            .pipe(_rebase())
            .pipe(makdoc.utils.dest(makdoc.vars.DIST()))
            .pipe($.size())
            .pipe($.connect.reload());
    }
...
||||||| parent of 015f3c8... move makdoc-utils functions to makdoc-utils.js
            .pipe(makdoc.util.dest(makdoc.vars.DIST()))
=======
            .pipe(makdoc.utils.dest(makdoc.vars.DIST()))
>>>>>>> 015f3c8... move makdoc-utils functions to makdoc-utils.js
            .pipe($.size())
            .pipe($.connect.reload());
    }
